...
class ThreePhaseTransformer(Transformer):
    ...
    def _get_loss_mc(self) -> float:
        ...
        return (0.5 / self._c2) * (
            self._c2 * self.specs.load_loss_hv_mv
            - self.specs.load_loss_hv_lv
            + self.specs.load_loss_mv_lv
        )
    ...
    def _end_temperature_top_oil(self, input_profile: np.ndarray) -> np.ndarray:
        ...
        hv_rise = self._get_loss_hc() * np.power(input_profile[0] / self.specs.hv_winding.nom_load, 2)
        mv_rise = self._get_loss_mc() * np.power(input_profile[1] / self.specs.mv_winding.nom_load, 2)
        lv_rise = self._get_loss_lc() * np.power(input_profile[2] / self.specs.lv_winding.nom_load, 2)

        end_temp_top_oil = self._pre_factor * (
            hv_rise + mv_rise + lv_rise + self.specs.no_load_loss
        ) / self.specs.load_loss_total
